...
class EclipseStoreAuthoritiesFetcherTest {
    ...
    @Test
    void authoritiesFetcher(AuthoritiesFetcher authoritiesFetcher, EclipseStoreUser registerService) throws UserAlreadyExistsException {
        assertThat(authoritiesFetcher.findAuthoritiesByEmail(NOT_FOUND_EMAIL))
            .isEmpty();
        registerService.register(new UserSave(FOUND_EMAIL, "password", Collections.emptyList()));
        assertThat(authoritiesFetcher.findAuthoritiesByEmail(FOUND_EMAIL))
            .isEmpty();
        registerService.register(new UserSave(FOUND_EMAIL_WITH_ROLES, "password", Collections.singletonList("ROLE_USER")));
